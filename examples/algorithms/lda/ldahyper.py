...
def learnParameters(alpha, locTopicDocCounts
    , locDocLengthCounts, shape=1.001, scale=1.0, numIterations=1):
    i = 0
    k = 0
    ...
    for itr in range(numIterations):
        ...
        for i in range(locDocLengthCounts.shape[0]):
            currentDigamma += 1.0 / ((parametersSum + np.float64(i) - 1.0) + 1e-100)
...
def learnSymmetricConcentration(countHistogram, topicSizeHistogram, numTypes, betaSum, optitr=200):
    currentDigamma = 0.0
...
def optimizeAlpha(topicDocCounts, docLengthCounts, maxTokens
    ...
    if usingSymmetricAlpha:
        ...
        alphaSum = learnSymmetricConcentration(locTopicDocCounts[0,:]
            , locDocLengthCounts, numTopics, alphaSum)
    else:
        locTopicDocCounts[:,:] = topicDocCounts[:,:]
        alphaSum =learnParameters(alpha
            , locTopicDocCounts, locDocLengthCounts, 1.001, 1.0, 1)

    return alphaSum

def optimizeBeta(typeTopicCounts, tokensPerTopic, numTypes, betaSum, maxTypeCount):
    countHistogram = np.zeros(np.int64(np.round(maxTypeCount))+1, dtype=np.int64)
    ...
    topicSizeHistogram = np.zeros(np.int64(np.round(maxTopicSize))+1, dtype=np.int64)

    for t in range(tokensPerTopic.shape[0]):
        topicSizeHistogram[np.int64(np.round(tokensPerTopic[t]))] += 1

    betaSum = learnSymmetricConcentration(countHistogram
...
def estimate(training, numTopics, alpha_, beta_
    ...
    topicSeq = list( [ randint(0, numTopics
        , np.sum(training[i, training_indices[i,:].mask]))
        for i in range(training_indices.shape[0]) ] )

    for i in range(training_indices.shape[0]):
        typ_idx = np.where(training_indices[i,:].mask == True)[0]
    ...
    for itr in range(numIterations):
        ...
        for j in range(training.shape[0]):
            ...
            oneDocTypes = np.where(training_indices[i,:].mask == True)[0]
            oneDocTypesFreq = training[i,training_indices[i,:].mask]
            for t in oneDocTopics[ np.where(oneDocTopics != UNASSIGNED_TOPIC)[0] ]:
            ...
            denseIdx = 0
            score = 0.0
            oneDocIndices = np.zeros(oneDocTypesFreq.shape[0]+1, dtype=np.uint64)
            ...
            for pos in range(oneDocIndices.shape[0]-1):
                ...
                for pos_range, old_topic, type_, type_freq in \
                    zip(pos_rng, topics, types, types_freq):
                    ...
                    while idx_ < currentTypeTopicCounts.shape[0] and \
                        ...
                        if not alreadyDecremented and currentTopic == old_topic:
                            ...
                            if currentValue == 0:
                                ...
                                currentTypeTopicCounts[old_topic] = currentValue
                            subidx = idx_
                            tmp = 0
                    ...
                    if sample < topicTermMass:
                        ...
                        currentTypeTopicCounts[:] = \
                             currentTypeTopicCounts[np.argsort(currentTypeTopicCounts)]
                    else:

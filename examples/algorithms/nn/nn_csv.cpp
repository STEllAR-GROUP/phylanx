    ...
    define(nn, X, y, num_iter, lr,
        block(
            ...
            for_each(lambda(i, block(
                define(hidden_layer_activations, __div(1, __add(1, exp(__minus(__add(
                dot(X, wh), bh)))))),
                define(output, __div(1, __add(1, exp(__minus(__add(dot(
                hidden_layer_activations, wout), bout)))))),
                define(Error, __sub(y, output)),
                define(d_output, __mul(Error, __mul(output, __sub(1, output)))),
                define(d_hidden_layer, __mul(dot(d_output, transpose(wout)), __mul(
                hidden_layer_activations, __sub(1, hidden_layer_activations)))),
                store(wout, __add(wout, __mul(dot(transpose(hidden_layer_activations),
                d_output), lr))),

...
{
    std::string commit = PHYLANX_HAVE_GIT_COMMIT;
    std::string location = boost::inspect::relative_to(
        full_path, boost::inspect::search_root_path() );
    // The erase function for location is to get rid of the first /phylanx that will always
    // be present in any full path this tool is used for (repeated in wordlink and
    // loclink)
    std::string total = "<a href = \"https://github.com/STEllAR-GROUP/phylanx/blob/"

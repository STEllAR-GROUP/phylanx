...
const char* phylanx_no_inspect = "phylanx-" "no-inspect";
    ...
    const path & dir_path, const inspector_list & insps )
  {
    ...
    for ( DirectoryIterator itr( dir_path ); itr != end_itr; ++itr )
    {
      ...
      else if (itr->path().filename().string()[0] != '.') // ignore if hidden
      {
        ...
        if (content.find(phylanx_no_inspect) == string::npos)
...
#endif
{
    ...
    pd.add("phylanx:positional", -1);

    options_description cmdline;
    cmdline.add(desc_commandline);
    cmdline.add_options()
        ("phylanx:positional", value<std::vector<std::string> >()->composing(),
            "positional options")
        ;
...
void print_output(std::ostream& out, inspector_list const& inspectors)
{
  ...
  if (display_format == display_text)
  {
    out
      <<
        "Phylanx Inspection Report\n"
        "Run Date: " << run_date  << "\n"
        "Commit: " << "<a href = \"https://github.com/STEllAR-GROUP/phylanx/commit/"
                   << PHYLANX_HAVE_GIT_COMMIT
                   << "\">"
                   << std::string(PHYLANX_HAVE_GIT_COMMIT, 10)
                   << "</a>\n"
        "\n"
      ;
  ...
  else
  {
    ...
    out
      ...
      "<title>Phylanx Inspection Report</title>\n"
      "</head>\n"
      ...
      "<tr>\n"
      "<td>"
      "<a href = \"https://github.com/STEllAR-GROUP/phylanx\">"
      "<img src=\"http://stellar.cct.lsu.edu/files/stellar100.png\""
      " alt=\"STE||AR logo\" />"
      "</a>\n"
      "</td>\n"
      "<td>\n"
      "<h1>Phylanx Inspection Report</h1>\n"
      "<b>Run Date:</b> " << run_date  << "<br>\n"
      //"&nbsp;&nbsp;/ " << validator_link( "validate me" ) << " /\n"
      "<b>Commit:</b> "
            << "<a href = \"https://github.com/STEllAR-GROUP/phylanx/commit/"
            << PHYLANX_HAVE_GIT_COMMIT
            << "\">"
            << std::string(PHYLANX_HAVE_GIT_COMMIT, 10)
            << "</a>\n"
      ...
      "program that checks files for the problems noted below. The Phylanx inspect "
...
//     display_worst_offenders(out);
  ...
  if ( !msgs.empty() )
  {
    ...
    {
      out << "Details:\n" << inspector_keys;
      out << "\nDirectories with a file named \"" << phylanx_no_inspect
          << "\" will not be inspected.\n" "Files containing \""
          << phylanx_no_inspect << "\" will not be inspected.\n";
   }
    else
    {
      out << "<h2>Details</h2>\n" << inspector_keys;
      out << "\n<p>Directories with a file named \"" << phylanx_no_inspect
          << "\" will not be inspected.<br>\n" "Files containing \""
          << phylanx_no_inspect << "\" will not be inspected.</p>\n";
    }
    display_details(out);
  }

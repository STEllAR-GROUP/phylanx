    ...
    namespace detail
    {
            ...
            expression const& replace)
        {
            ...
            case 1:     // phylanx::util::recursive_wrapper<primary_expr>
                return transduce(util::get<1>(op.var).get(), match, replace);

            case 2:     // phylanx::util::recursive_wrapper<unary_expr>
                return transduce(util::get<2>(op.var).get(), match, replace);
            ...
            operand const& op, transform_rule const& rule, bool& found_expr)
        {
            ...
            {
            case 1:     // phylanx::util::recursive_wrapper<primary_expr>
                return transform(util::get<1>(op.var).get(), rule, found_expr);

            case 2:     // phylanx::util::recursive_wrapper<unary_expr>
                return transform(util::get<2>(op.var).get(), rule, found_expr);
        ...
        operand simplify(operand const& op)
        {
                    ...
                    if (pe.var.index() == 6)
                    {
                        ...
                        if (expr.rest.empty())
                        {
                            ...
                            case 2:
                                ...
                                return simplify(util::get<2>(expr.first.var).get());
            ...
            case 2:     // phylanx::util::recursive_wrapper<unary_expr>
                return simplify(util::get<2>(op.var).get());

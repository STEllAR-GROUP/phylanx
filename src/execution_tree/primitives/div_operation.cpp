...
struct DivScalar_nd0d
{
public:
    explicit inline DivScalar_nd0d( double scalar )
            : scalar_( scalar )
    {}
    ...
    template< typename T >
    static constexpr bool simdEnabled() { return blaze::HasSIMDDiv<T,double>::value; }
...
struct DivScalar_0dnd
{
public:
    explicit inline DivScalar_0dnd( double scalar )
            : scalar_( scalar )
    {}
            ...
            namespace detail
            {
                ...
                protected:
                    ...
                    primitive_result_type div0d1d(operands_type && ops) const
                    {
                        ...
                        ops[1].vector() = blaze::map(ops[1].vector(),DivScalar_0dnd(ops[0].scalar()));
                    ...
                    primitive_result_type div0d2d(operands_type && ops) const
                    {
                        ...
                        ops[1].matrix() = blaze::map(ops[1].matrix(),DivScalar_0dnd(ops[0].scalar()));
                    ...
                    primitive_result_type div1d0d(operands_type && ops) const
                    {
                        ...
                        ops[0].vector() = blaze::map(ops[0].vector(),DivScalar_nd0d(ops[1].scalar()));
                    ...
                    primitive_result_type div1d1d(operands_type && ops) const
                    {
                        ...
                        if (ops.size() == 2)
                        {
                            lhs.vector() = blaze::map(lhs.vector(),rhs.vector(),Div());
                            return primitive_result_type(std::move(lhs));
                        }
                    ...
                    primitive_result_type div2d0d(operands_type && ops) const
                    {
                        ...
                        ops[0].matrix() = blaze::map(ops[0].matrix(),DivScalar_nd0d(ops[1].scalar()));
                        return primitive_result_type(std::move(ops[0]));
                    }
                    ...
                    primitive_result_type div2d2d(operands_type && ops) const
                    {
                        ...
                        if (ops.size() == 2)
                        {
                            lhs.matrix() = blaze::map(lhs.matrix(),rhs.matrix(),Div());
                            return primitive_result_type(std::move(lhs));
                        }
                        ...
                        return primitive_result_type(std::accumulate(
                                ...
                                [](operand_type& result, operand_type const& curr)
                                        ->  operand_type
                                {
                                    result.matrix() = blaze::map(result.matrix(),curr.matrix(),Div());
                                    return std::move(result);
                                }));
                    }

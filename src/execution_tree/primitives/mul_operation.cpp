...
namespace phylanx { namespace execution_tree { namespace primitives
{
      ...
      , operands_(std::move(operands))
    {
        ...
        if (!arguments_valid)
        {
            HPX_THROW_EXCEPTION(hpx::bad_parameter,
                "mul_operation::mul_operation",
                "the mul_operation primitive requires that there is exactly "
                    "one element of the literals and operands arrays is valid");
        }
    }
    ...
    namespace detail
    {
        ...
        auto map(std::vector<T1> const& in1, std::vector<T2> const& in2,
            F&& f) -> std::vector<decltype(hpx::util::invoke(f,
            std::declval<T1>(), std::declval<T2>()))>
        {
            HPX_ASSERT(in1.size() == in2.size());

            std::vector<decltype(hpx::util::invoke(
                f, std::declval<T1>(), std::declval<T2>()))>
                out;

...
#include <phylanx/plugins/common/conv1d_all_paddings_bita.hpp>
...
namespace phylanx { namespace dist_keras_support { namespace primitives {
    ///////////////////////////////////////////////////////////////////////////
    execution_tree::match_pattern_type const dist_conv1d_bita::match_data = {
        hpx::util::make_tuple("conv1d_d_bita", std::vector<std::string>{R"(
            conv1d_d_bita(_1, _2_kernel,
            __arg(_3_padding, "valid"),
            __arg(_4_strides, 1),
            __arg(_5_dilation_rate, 1),
            __arg(_6_name, ""))
        )"},
            &create_dist_conv1d_bita,
            &execution_tree::create_primitive<dist_conv1d_bita>,
    ...
    ///////////////////////////////////////////////////////////////////////////
    dist_conv1d_bita::dist_conv1d_bita(
    ...
    dist_conv1d_bita::conv1d_all_paddings(ir::node_data<double>&& arg,
        ...
        std::string&& padding, std::string&& given_name) const
    {
        ...
        if (numtiles > 0 && numtiles_k == 1)
        {
            ...
            if (padding == "valid" || arg_locs.is_page_tiled(name_, codename_))
            {
                ...
                local_result = common::conv1d_all_paddings_bita(
        ...
        HPX_THROW_EXCEPTION(hpx::bad_parameter,
            "dist_conv1d_bita::conv1d_all_paddings",
        ...
        std::string&& given_name) const
    {
        ...
        return common::conv1d_all_paddings_bita(
            extract_numeric_value(std::move(arg), name_, codename_),
            extract_numeric_value(std::move(kernel), name_, codename_),
            std::move(padding), name_, codename_);
    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    hpx::future<execution_tree::primitive_argument_type> dist_conv1d_bita::eval(
        ...
        execution_tree::eval_context ctx) const
    {
        ...
        {
            HPX_THROW_EXCEPTION(hpx::bad_parameter, "dist_conv1d_bita::eval",
            ...
            hpx::util::unwrapping(
                [this_ = std::move(this_)](
                        ...
                        ndim != 3)
                    {
                        HPX_THROW_EXCEPTION(hpx::bad_parameter,
                            ...
                            this_->generate_error_message(
                                "conv1d_d operation requires for x and kernel "
                                "to "
                                "be "
                                "tensors"));
                    }
                    ...
                    if (padding == "valid")
                    {
                                ...
                                args[1], this_->name_, this_->codename_)[0])
                        {
                            HPX_THROW_EXCEPTION(hpx::bad_parameter,
                                "dist_conv1d_bita::eval",
                                this_->generate_error_message(
                                    "the kernel size cannot be greater than "
                                    "the "
                    ...
                    HPX_THROW_EXCEPTION(hpx::bad_parameter,
                        "dist_conv1d_bita::eval",
                        this_->generate_error_message(
                            "strides > 1 or dilation_rate > 1 are not "
                            "currently "
                            "supported"));
                }),

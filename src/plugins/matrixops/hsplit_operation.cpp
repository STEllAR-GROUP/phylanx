...
///////////////////////////////////////////////////////////////////////////
    match_pattern_type const hsplit_operation::match_data =
    {
        match_pattern_type{
            "hsplit",
            std::vector<std::string>{"hsplit(_1, _s_2)"},
    ...
    /*
        template<typename T>
        primitive_argument_type hsplit_operation::hsplit1d_helper(
            primitive_arguments_type&& args) const
        {
            ...
            if (block_dims < 1)    // Scalar tiling
            {
                ...
                // smallest
                for(int i = 0; i < num_cols-1; i += block_size)
                {
            ...
            else    // Index-based tiling
            {
                ...
                if (v.size() > 0)
                {
                    ...
                    {
                        ranges.push_back(std::make_pair<std::size_t,
       std::size_t>(
            ...
            primitive_arguments_type&& args) const
        {
            ...
            case node_data_type_bool:
                return hsplit1d_helper<std::uint8_t>(std::move(args));

            case node_data_type_int64:
                return hsplit1d_helper<std::int64_t>(std::move(args));

            case node_data_type_double:
                return hsplit1d_helper<double>(std::move(args));
            ...
            HPX_THROW_EXCEPTION(hpx::bad_parameter,
                "phylanx::execution_tree::primitives::"
                    "hsplit_operation::hsplit1d",
                generate_error_message(
                    "the hsplit_operation primitive requires for all arguments
       to "
    ...
    */
    ...
    hsplit_operation::hsplit2d_helper(primitive_arguments_type &&args) const {
      std::array<std::size_t, PHYLANX_MAX_DIMENSIONS> matrix_dims =
          extract_numeric_value_dimensions(args[0], name_, codename_);
      ...
      if (block_dims < 1) // Scalar tiling
      {
        ...
        if (num_blocks <= 0) {
          HPX_THROW_EXCEPTION(
              hpx::bad_parameter, "hsplit_operation::eval",
              generate_error_message(
                  "the hsplit_operation primitive can not split "
                  "matrices/vectors into fewer blocks than one"));
        }
        ...
        if (ranges[i].second > ranges[i].first) {
          ...
          for (int j = 0; j < num_rows; j++) {
            for (int k = 0; k < diff; k++) {
              block(j, k) = m(j, ranges[i].first + k);
            }
          }
    ...
    primitive_argument_type
    hsplit_operation::hsplit2d(primitive_arguments_type &&args) const {
      switch (extract_common_type(args[0])) {
      case node_data_type_bool:
      ...
      HPX_THROW_EXCEPTION(
          hpx::bad_parameter, "phylanx::execution_tree::primitives::"
    ...
    ///////////////////////////////////////////////////////////////////////////
    primitive_argument_type
    hsplit_operation::hsplit_args(primitive_arguments_type &&args) const {
      std::size_t matrix_dims =
          extract_numeric_value_dimension(args[0], name_, codename_);

      switch (matrix_dims) {
      // case 1:
      // return hsplit1d(std::move(args));
      case 2:
        return hsplit2d(std::move(args));

      default:
        HPX_THROW_EXCEPTION(
            hpx::bad_parameter, "hsplit_operation::hsplit_args",
    ...
    hsplit_operation::eval(primitive_arguments_type const &operands,
      ...
      if (!arguments_valid) {
        HPX_THROW_EXCEPTION(
            hpx::bad_parameter, "hsplit_operation::eval",
            generate_error_message("the hsplit_operation primitive requires "

...
namespace phylanx { namespace execution_tree { namespace primitives {
    ...
    namespace detail {
        extract_shape::shape_mode extract_shape_mode(std::string const& name)
        {
            ...
            if (name.find("shape_d") != std::string::npos)
            {
    ...
    extract_shape::extract_shape(primitive_arguments_type&& operands,
    ...
    {
        HPX_THROW_EXCEPTION(hpx::bad_parameter, "extract_shape::shape0d",
        ...
        primitive_argument_type&& arg, std::int64_t index) const
    {
        if (index != 0 && index != -1)
        {
            HPX_THROW_EXCEPTION(hpx::bad_parameter, "extract_shape::shape1d",
                generate_error_message("index out of range"));
        }
        ...
        primitive_argument_type&& arg, std::int64_t index) const
    {
        if (index < -2 || index >= 2)
        {
            HPX_THROW_EXCEPTION(hpx::bad_parameter, "extract_shape::shape2d",
                generate_error_message("index out of range"));
        }
        ...
        primitive_argument_type&& arg, std::int64_t index) const
    {
        if (index < -3 || index >= 3)
        {
            HPX_THROW_EXCEPTION(hpx::bad_parameter, "extract_shape::shape3d",
                generate_error_message("index out of range"));
        }
        ...
        primitive_argument_type&& arg, std::int64_t index) const
    {
        if (index < -4 || index >= 4)
        {
            HPX_THROW_EXCEPTION(hpx::bad_parameter, "extract_shape::shape4d",
                generate_error_message("index out of range"));
        }
        ...
        primitive_arguments_type const& args, eval_context ctx) const
    {
        ...
        {
            HPX_THROW_EXCEPTION(hpx::bad_parameter, "extract_shape::eval",
                generate_error_message("the extract_shape primitive requires "
                                       "one or two operands"));
        }
        ...
        return hpx::dataflow(
            ...
            [this_ = std::move(this_)](
                hpx::future<primitive_argument_type>&& farg,
                hpx::future<std::int64_t>&& findex) -> primitive_argument_type {
                auto&& arg = farg.get();

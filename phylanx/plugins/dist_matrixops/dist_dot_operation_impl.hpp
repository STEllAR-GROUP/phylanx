...
////////////////////////////////////////////////////////////////////////////////
namespace phylanx {
    namespace dist_matrixops { namespace primitives {
        ////////////////////////////////////////////////////////////////////////////
        template <typename T>
        ...
        {
            if (lhs_localities.dimension() < 1 ||
                rhs_localities.dimension() < 1)
            {
                HPX_THROW_EXCEPTION(hpx::bad_parameter,
            ...
            execution_tree::localities_information const& rhs_localities) const
        {
            ...
            case 1:
                ...
                return dot1d1d(std::move(lhs), std::move(rhs), lhs_localities,
                    rhs_localities);
            ...
            execution_tree::localities_information const& rhs_localities) const
        {
            ...
            for (auto const& rhs_tile : rhs_localities.tiles_)
            {
                ...
                {
                    // calculate the dot product with local tile
                    dot_result += blaze::submatrix(lhs.matrix(), 0,
                                      lhs_intersection.start_, lhs.dimension(0),
                                      lhs_intersection.size()) *
                        blaze::subvector(*rhs_data, rhs_intersection.start_,
                            rhs_intersection.size());
                }
            ...
            execution_tree::localities_information const& rhs_localities) const
        {
            ...
            // go over all tiles of rhs vector, the result size is determined by the
            // number of rows of the lhs tile
            //blaze::DynamicVector<T> dot_result{1};    //, {1}};
            // An optimization is that the local result matrix only has as many rows as the
            // LHS tile does, not as many as the entire LHS has. But this is only a side
            // -effect of this algorithm, I think it could also be the reverse if the LHS
            // tiles were retrieved instead of the RHS
                ...
                ir::node_data<T> && lhs, ir::node_data<T> && rhs) const
            {
                ...
                blaze::DynamicTensor<T> result(
                    m.rows(), t.pages(), t.columns());

                for (std::size_t i = 0; i != t.pages(); ++i)
                    blaze::rowslice(blaze::subtensor(result, 0, i, 0, m.rows(),
                                        1, t.columns()),
            ...
            execution_tree::primitive_argument_type dist_dot_operation::dot2d(
                ir::node_data<T> && lhs, ir::node_data<T> && rhs,
                execution_tree::localities_information && lhs_localities,
                execution_tree::localities_information const& rhs_localities)
                const
            {
                switch (rhs.num_dimensions())
                {
                ...
                ir::node_data<T> && lhs, ir::node_data<T> && rhs) const
            {
                ...
                blaze::DynamicTensor<T> result(
                    t.pages(), t.rows(), m.columns());

                for (std::size_t i = 0; i != t.pages(); ++i)
                    blaze::pageslice(blaze::subtensor(result, i, 0, 0, 1,
                                         t.rows(), m.columns()),

...
///////////////////////////////////////////////////////////////////////////////
char const* const conv1d_test_code_shahrzad = R"(
    define(conv1d_test_shahrzad, input, filter1, filter2, filter3,
    block(
        define(kernel, random(list(filter1, filter2, filter3))),
        define(out, conv1d_d_shahrzad(input, kernel)),
        out
       )
    )
    conv1d_test_shahrzad
)";

///////////////////////////////////////////////////////////////////////////////
char const* const conv1d_test_code_bita = R"(
    define(conv1d_test_bita, input, filter1, filter2, filter3,
    block(
        define(kernel, random(list(filter1, filter2, filter3))),
        define(out, conv1d_d_bita(input, kernel)),
        out
       )
    )
    conv1d_test_bita
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    auto const& conv1d_test_compile_shahrzad = phylanx::execution_tree::compile(
        "conv1d_test_shahrzad", conv1d_test_code_shahrzad, snippets);
    auto const& conv1d_test_compile_bita = phylanx::execution_tree::compile(
        "conv1d_test_bita", conv1d_test_code_bita, snippets);
    ...
    auto conv1d_test_shahrzad = conv1d_test_compile_shahrzad.run();
    auto conv1d_test_bita = conv1d_test_compile_bita.run();
    ...
    auto channels = vm["channels"].as<std::int64_t>();
    ...
    if (mode == "shahrzad")
    {
        auto input = conv1d_input(batch, channels, length);
        // Measure execution time
        hpx::util::high_resolution_timer t;

        // Evaluate ALS using the read data
        auto result =
            conv1d_test_shahrzad(input, out_channels, channels, filter_length);
        auto elapsed = t.elapsed();
        std::cout << "time: " << t.elapsed() << std::endl;

        auto output = phylanx::execution_tree::extract_numeric_value(result);
        auto output_t = output.tensor();
        std::cout << "[" << output_t.pages() << "," << output_t.rows() << ","
                  << output_t.columns() << "]" << std::endl;
    ...
    {
        auto input = conv1d_input(batch, length, channels);
        // Measure execution time
        hpx::util::high_resolution_timer t;

        // Evaluate ALS using the read data
        auto result =
            conv1d_test_bita(input, filter_length, channels, out_channels);
        auto elapsed = t.elapsed();
        std::cout << "time: " << t.elapsed() << std::endl;

        auto output = phylanx::execution_tree::extract_numeric_value(result);
        auto output_t = output.tensor();
    	std::cout << "[" << output_t.pages() << "," << output_t.columns() << ","
                  << output_t.rows() << "]" << std::endl;
    ...
    else
    {
        ...
        // Evaluate ALS using the read data
        auto result = conv1d_test(input, filter_length, channels, out_channels);
        auto elapsed = t.elapsed();
        std::cout << "time: " << t.elapsed() << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()(
        ...
        "batch,b", hpx::program_options::value<std::int64_t>(), "batch")(
        "length,l", hpx::program_options::value<std::int64_t>(), "length")(
        "channels,c", hpx::program_options::value<std::int64_t>(), "channels")(
        "filter_length,f", hpx::program_options::value<std::int64_t>(),
        "filter length")("out_channels,o",
        hpx::program_options::value<std::int64_t>(), "kernel out")("mode,m",
        hpx::program_options::value<std::string>()->default_value("master"),
        "mode");
